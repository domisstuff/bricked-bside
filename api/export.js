...
export default async function handler(req, res) {
  ...
  const batch_size = 50 // fetch 50 keys per batch
  ...
  try {
    ...
    while (true) {
      const r = await axios.get(
        ...
        { headers: { "x-api-key": api_key }, params: { datastoreName: datastore, limit: batch_size, cursor } }
      )
      ...
      for (let k of keys) {
        try {
          ...
          console.warn("failed to fetch key:", k)
        }
      }
